...
Write-Host "Install=$Install NoDownload=$NoDownload SkipEDA=$SkipEDA SkipBacktest=$SkipBacktest SkipTrain=$SkipTrain Symbols=$Symbols"
...
if ($Install) {
    Write-Host "[1/9] Updating pip and installing project dependencies..." -ForegroundColor Yellow
    ...
    Write-Host "[1/9] Skipping install (use -Install to enable)" -ForegroundColor DarkGray
}

# Download data (unless skipped)
if (-not $NoDownload) {
    Write-Host "[2/9] Downloading data via src.data.download ..." -ForegroundColor Yellow
    & $PY -m src.data.download
    if ($LASTEXITCODE -ne 0) { throw "data download failed" }
}
else {
    Write-Host "[2/9] Skipping data download (NoDownload)" -ForegroundColor DarkGray
}

# EDA
if (-not $SkipEDA) {
    Write-Host "[3/9] Running EDA notebooks/eda_altcoins.py ..." -ForegroundColor Yellow
    & $PY notebooks/eda_altcoins.py
    if ($LASTEXITCODE -ne 0) { throw "EDA failed" }
}
else {
    Write-Host "[3/9] Skipping EDA" -ForegroundColor DarkGray
}

# Backtest
if (-not $SkipBacktest) {
    Write-Host "[4/9] Backtest: src.backtest.run_altcoins ..." -ForegroundColor Yellow
    & $PY -m src.backtest.run_altcoins
    if ($LASTEXITCODE -ne 0) { throw "backtest failed" }
}
else {
    Write-Host "[4/9] Skipping backtest" -ForegroundColor DarkGray
}

# Train LightGBM
if (-not $SkipTrain) {
    Write-Host "[5/9] Training LightGBM per symbol ..." -ForegroundColor Yellow
    if ($Symbols -ne "ALL") {
    ...
    Write-Host "[5/9] Skipping training" -ForegroundColor DarkGray
}
...
Write-Host "[6/9] Calibrating probability thresholds ..." -ForegroundColor Yellow
& $calibCmd
if ($LASTEXITCODE -ne 0) { throw "probability calibration failed" }

# Summaries
Write-Host "[7/9] Summaries:" -ForegroundColor Green
...
Write-Host "[8/9] Calibration Backtest ..." -ForegroundColor Cyan
& $PY -m src.backtest.run_calibrated
if ($LASTEXITCODE -ne 0) { throw "calibration backtest failed" }

Write-Host "[9/9] Compare & Report ..." -ForegroundColor Cyan
