...
def load_config(path: Path | None) -> tuple[dict, dict, list[str]]:
    if path is None:
        cfg_path = ROOT / "configs" / "strategy_confidence.yaml"
    else:
        cfg_path = path
...
def main(
    ...
    *,
    ...
    defaults = {**defaults, **overrides}  # CLI overrides highest priority
    ...
    for sym in symbols:
        ...
        res = run_for_symbol(
            sym,
            p_enter=float(params.get("p_enter", 0.55)),
            p_exit=float(params.get("p_exit", 0.5)),
            max_std=float(params.get("max_std", 0.1)),
        ...
        if res is None:
            rows.append(
                {"symbol": sym, "variant": "confidence_long_v1", "error": "no inference data"}
            )
            continue
        rows.append(res)

    df = pd.DataFrame(rows)
    out_csv = OUT_DIR / "metrics_confidence_long.csv"
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to YAML config (default configs/strategy_confidence.yaml)",
    )
    ap.add_argument("--p-enter", type=float, help="Override p_enter")
    ap.add_argument("--p-exit", type=float, help="Override p_exit")
